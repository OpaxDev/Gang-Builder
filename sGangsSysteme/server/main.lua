    ...
    if xPlayer.getGroup() ~= "user" then 
        if not SharedGangs[data.Name] then
            if not ESX.Jobs[data.Name] then 
                ...
                Notification(_source, "~r~Creation de gang~s~\nVous avez cree le gang "..data.Label..".")
            else
                Notification(_source, "~r~Creation de gang~s~\nCe gangs existe dejà.")
            end
        else
            Notification(_source, "~r~Creation de gang~s~\nCe gangs existe dejà.")
        end
    ...
    if xJob.name == job then 
        if SharedGangs[job].data["item"] then 
            ...
            if itemInfo.count >= count then 
                ...
                if (Config.CoffrePoidsMax - CurrentWeight) >= (itemInfo.weight*count) then
                    ...
                    Notification(_source, "~g~Coffre "..SharedGangs[job].label.."\n~s~Vous avez depose "..count.." "..itemInfo.label.." dans le coffre.")
    ...
    if xJob.name == job then 
        ...
        if SharedGangs[job].data["item"] and SharedGangs[job].data["item"][itemInfo.name] then 
            if SharedGangs[job].data["item"][itemInfo.name].count >= count then 
                if xPlayer.canCarryItem(itemInfo.name, count) then
                    ...
                    Notification(_source, "~g~Coffre "..SharedGangs[job].label.."\n~s~Vous avez pris "..count.." "..itemInfo.label.." du coffre.")
                    TriggerClientEvent("sGangsSysteme:UpdateCoffre", _source, SharedGangs[job].data)
                else 
                    Notification(_source, "~g~Coffre "..SharedGangs[job].label.."\n~s~Vous pouvez pas prendre cette quantitee !")
                end
            else
                Notification(_source, "~g~Coffre "..SharedGangs[job].label.."\n~s~Il n'y a pas cette quantite dans le coffre !")
            end
        end
    else
...
AddEventHandler("sGangsSysteme:AddMoney", function(job, moneyType, amount) 
    ...
    if xJob.name == job then 
        ...
        if Account.money >= amount then
            ...
            Notification(_source, "~g~Coffre "..SharedGangs[job].label.."~s~\nVous avez deposer "..amount.." de "..Account.label.." dans le coffre.") 
            TriggerClientEvent("sGangsSysteme:UpdateCoffre", _source, SharedGangs[job].data)
        else 
            Notification(_source, "~g~Coffre "..SharedGangs[job].label.."~s~\nVous n'avez pas cette quantitee.") 
        end
    else 
...
AddEventHandler("sGangsSysteme:SuppMoney", function(job, moneyType, amount) 
    ...
    if xJob.name == job then 
        if SharedGangs[job].data["accounts"][moneyType] >= amount then
            ...
            Notification(_source, "~g~Coffre "..SharedGangs[job].label.."~s~\nVous avez pris "..amount.." de "..AccountsLabel.." dans le coffre.") 
            TriggerClientEvent("sGangsSysteme:UpdateCoffre", _source, SharedGangs[job].data)
        else 
